...
def when_pigs_fly(score, num_rolls):
    """Return the smaller score between the player score and 25 minus the number of rolls
    """
...
#        if(strategy(score, opponent_score) == -1):
#            dice_swapped = not dice_swapped
#            select_dice(score, opponent_score, dice_swapped)
#            return 1
#        else:
#            return take_turn(strategy(score, opponent_score), opponent_score, select_dice(score, opponent_score, dice_swapped))
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    while(i<=goal):
        n = 0
        while(n<=goal):
            check_strategy_roll(i, n, strategy(i, n))
            n += 1
        i+=1
    # END PROBLEM 6
